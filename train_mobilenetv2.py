...
def main():
    ...
    VOC_root = "E:/AC-FPN/coco2017"  # VOCdevkit
    aspect_ratio_group_factor = 3
    batch_size = 8
    amp = False  # 是否使用混合精度训练，需要GPU支持

    # check voc root
    if os.path.exists(os.path.join(VOC_root, "coco2017")) is True:
        raise FileNotFoundError("coco2017 dose not in path:'{}'.".format(VOC_root))

    # load train data set
    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> train.txt
    train_dataset = CocoDetection(VOC_root, "train", data_transform["train"])
    train_sampler = None
    ...
    val_dataset = CocoDetection(VOC_root, "val", data_transform["val"])
