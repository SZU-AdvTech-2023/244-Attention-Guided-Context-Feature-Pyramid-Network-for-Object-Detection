...
# 包含了一些在目标检测中常用的工具和损失函数

# 用于在训练过程中对正负样本进行平衡的采样。通过指定每张图像中选取的元素数量 (batch_size_per_image) 和正样本在每个批次中的比例 (positive_fraction)。
# __call__ 方法接受一个列表 matched_idxs，该列表包含每个图像的匹配索引（-1，0，或正值）。根据这些匹配索引，生成每个图像中正负样本的二进制掩码。
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs):
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            rep03 = positive03[torch.randperm(positive03.numel(), device=positive.device)[:num_pos_k]]
            ...
            ref03 = negative03[torch.randperm(negative03.numel(), device=negative.device)[:num_neg_k]]
...
# 用于将一组 proposals 相对于一组 reference boxes 进行编码。编码过程涉及计算中心坐标、宽度和高度的回归参数。
# 该函数将 proposals、reference boxes 和权重作为输入，并输出编码后的回归参数
...
# 用于编码和解码一组边界框。在目标检测中，这通常用于将 ground-truth 边界框与预测的边界框之间的差异编码为回归目标，以便在训练中进行优化。
...
# 用于将预测的边界框与 ground-truth 边界框匹配，产生匹配索引。
# 它基于预测边界框与 ground-truth 边界框之间的质量（例如，IoU）来确定匹配关系。
# 具有一些阈值，如高阈值和低阈值，以及是否允许低质量匹配的参数。
...
# 平滑 L1 损失函数。它计算输入 input 和目标 target 之间的平滑 L1 损失，通过参数 beta 控制平滑过渡的阈值。可用于回归任务中的损失计算。
