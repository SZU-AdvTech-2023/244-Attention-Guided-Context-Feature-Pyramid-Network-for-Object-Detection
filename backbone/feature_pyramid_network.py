...
# 实现了CxAM和CnAM模块
"""
该模块定义了一个类，用于实现上下文感知的注意力机制。
使用三个卷积层（key_conv、query_conv、value_conv）来生成查询、键和值。
利用这些查询、键和值计算注意力权重，然后将值与注意力权重相乘得到最终输出。
"""
...
"""
类似于CxAM，实现了上下文感知的非局部注意力机制。
接受两个输入 F5 和 F，通过计算注意力权重将 F 中的信息融合到 F5 中
"""
...
"""
该模块定义了一个稠密块，由一系列卷积层组成，包括 1x1 卷积、组归一化（Group Normalization）、ReLU 激活、带有空洞卷积的卷积、ReLU 激活和 Dropout。
用于构建DenseAPP模块。
"""
...
"""
该模块使用了多个DenseBlock模块，通过串联和融合不同空洞率的卷积层，构建了一个密集的上下文感知模块。
"""
...
"""
主要的特征金字塔网络模块，通过上采样和下采样操作，将不同尺度的特征图级联在一起。
包括一个DenseAPP模块，用于提取上下文信息。
包含CxAM和CnAM模块，用于引入上下文感知的注意力机制。
通过一系列卷积操作调整不同层次特征图的通道数，并通过插值操作将高层次的特征图上采样到低层次。
最终输出一个有序字典，包含不同尺度的特征图。
"""
