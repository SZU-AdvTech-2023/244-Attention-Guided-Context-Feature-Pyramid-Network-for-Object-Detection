...
# 提供一个用于目标检测等任务的强大的深度学习模型。
# ResNet提供了强大的特征提取能力，而FPN有助于处理不同尺度的目标。这对于目标检测等任务是非常有用的。

# 该类定义了ResNet中的瓶颈块，结构与之前提到的ResNet50的瓶颈块相似。
# 瓶颈块通过堆叠1x1、3x3和1x1的卷积层，以及批归一化和ReLU激活函数来增强网络的表示能力。
...
"""
该类定义了ResNet模型，包括多个不同层的堆叠。
与ResNet50相似，不同之处在于ResNet101包含更多的层。
模型的构建包括卷积层、批归一化、ReLU激活函数等。
"""
...
# 该类是一个模块包装器，用于从模型中提取中间层的输出。
# 通过正向传播过程中的层次结构，将指定层次的输出收集到一个有序字典中。
...
# 该类在给定的ResNet模型基础上添加了特征金字塔网络（FPN）。
# 使用IntermediateLayerGetter 从ResNet模型中提取中间层，并将这些中间层的输出输入到FPN中。
# FeaturePyramidNetwork（fpn）是一个额外的模块，用于生成多尺度的特征金字塔。
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self, backbone, return_layers, in_channels, out_channels, extra_blocks=None):
...
# 该函数构建了一个ResNet101模型，并添加了FPN。
# 可以选择是否加载预训练权重、设置不同的批归一化层（norm_layer）和冻结不同层的参数。
# 返回一个BackboneWithFPN 实例，其中包含了整个模型的结构。
def resnet101_fpn_backbone(pretrain_path="./backbone/resnet101.pth",
    ...
    if pretrain_path != "":
        ...
        # 载入预训练权重
        print("加载预训练权重：", resnet_backbone.load_state_dict(torch.load(pretrain_path), strict=False))
