...
# 构建具有特征金字塔网络（Feature Pyramid Network, FPN）的ResNet50模型

"""
该类定义了ResNet中的瓶颈块。
包括一个1x1的卷积层，一个3x3的卷积层，以及另一个1x1的卷积层，中间夹有批归一化（Batch Normalization）和ReLU激活函数。
瓶颈块的设计有助于提高网络的表示能力。
"""
...
"""
该类定义了ResNet模型的主体结构，包括多个不同层的堆叠。
在构建ResNet时，通过_make_layer 方法生成每个阶段的层。
可以选择是否包括分类头部（include_top），默认为包括。
"""
...
接收一个模型和一个字典，该字典指定要返回的中间层的名称和输出名称。
